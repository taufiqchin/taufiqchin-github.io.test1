...
function computeFitDimensions(img, boxW, boxH) {
  const imgW = img.naturalWidth;
  const imgH = img.naturalHeight;
...
function drawImageRounded(ctx, slot) {
  ...
  const { x, y, width, height, borderRadius, alignH, alignV, image, fitMode, rotation } = slot;
  const cx = x + width / 2;
  const cy = y + height / 2;
  ...
  if (fitMode) {
    const fit = computeFitDimensions(image, width, height);
    ctx.drawImage(image, -width / 2 + fit.dx, -height / 2 + fit.dy, fit.dw, fit.dh);
  } else {
...
function drawCircleGuide(ctx, bounds) {
...
function drawLogoGuide(ctx, w, h) {
...
function renderPoster(canvas, state, options = {}) {
  ...
  if (showOverlay && asset.profileMode) {
    drawCircleGuide(ctx, getProfileCircleBounds(w, h));
  }
  if (showOverlay && asset.logoMode) {
    drawLogoGuide(ctx, w, h);
  }
  if (showSelection) {
